...
AgentForge 是一个基于 LangChain / LangGraph 的 Agent 应用开发底座，目标是像若依之于 Spring Boot 一样，为 AI Agent 应用提供统一的工程结构、插件规范、工作流编排、模型接入、结构化协议和可测试业务示例。
...
本地优先的扩展模块。默认实现不依赖外部服务，适合作为业务项目的可替换底座。
